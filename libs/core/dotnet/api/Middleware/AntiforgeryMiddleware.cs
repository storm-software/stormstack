...
namespace OpenSystem.Core.Api.Middleware
{
    ...
    public class AntiforgeryMiddleware
    {
        ...
        public async Task Invoke(HttpContext httpContext)
        {
            ...
            if (
                ...
                && !httpContext.Items.ContainsKey(AntiforgeryMiddlewareInvokedKey)
            )
            {
                httpContext.Items.Add(
                    AntiforgeryMiddlewareInvokedKey,
                    AntiforgeryMiddlewareInvokedValue
                );

                if (
                    endpoint.Metadata.GetMetadata<IAntiforgeryMetadata>()
                    is IAntiforgeryMetadata metadata
                )
                {
