...
namespace OpenSystem.Core.Application.Helpers
{
  ...
  {
    public static string Merge(string originalJson,
      string newContent)
    {
          ...
          new JsonWriterOptions { Indented = true }))
        {
            ...
            if (root1.ValueKind != JsonValueKind.Array &&
              root1.ValueKind != JsonValueKind.Object)
            {
            ...
            if (root1.ValueKind == JsonValueKind.Array)
            {
                MergeArrays(jsonWriter,
                  root1,
                  root2);
            }
            else
            {
                MergeObjects(jsonWriter,
                  root1,
                  root2);
            }
        }
    ...
    private static void MergeObjects(Utf8JsonWriter jsonWriter,
      JsonElement root1,
      JsonElement root2)
    {
        ...
        foreach (JsonProperty property in root1.EnumerateObject())
        {
              ...
              (newValueKind = newValue.ValueKind) != JsonValueKind.Null)
            {
                ...
                else if (newValueKind == JsonValueKind.Array &&
        ...
        {
            if (!root1.TryGetProperty(property.Name,
              out _))
            {
                property.WriteTo(jsonWriter);
            }
    ...
    private static void MergeArrays(Utf8JsonWriter jsonWriter,
      JsonElement root1,
      JsonElement root2)
    {
