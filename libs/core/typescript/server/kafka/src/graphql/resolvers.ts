...
import { ddl } from "./ddl";
import { asyncIteratorQueryStream } from "./query-stream";
...
export const createInsertStatement = (
  ...
  const command = `INSERT INTO ${name} (${keys.join(
    ", "
  )}) VALUES (${values.join(", ")});`;
  return command;
};
...
export class ResolverGenerator {
  ...
  constructor(protected fields: ResolverFields) {
    ...
    queries
      .concat(subscriptions, mutations)
      .reduce((accum: Set<string>, key: any) => {
        let args;
        if (queryFields[key]) {
    ...
    this.queryResolvers = queries.reduce(
      createResolver(this.handleQueryResolve),
      {}
    );
    this.mutationResolvers = mutations.reduce(
      createResolver(this.handleMutationResolve),
      {}
    );
    this.subscriptionResolvers = subscriptions.reduce(
      (resolvers: SubscriptionResolver, key: string) => {
        resolvers[key] = {
          subscribe: this.handleSubscriptionResolve as any,
        };
        return resolvers;
      },
      {}
  ...
  /*
   *Creates a generic ksql statement based for graphql.
   */
  generateStatement = (
    info: GraphQLResolveInfo,
    ...
    const command = [];
    const selections: Array<FieldNode> = node.selectionSet
      .selections as Array<FieldNode>;
   ...
   */
  handleSubscriptionResolve: KsqlDBGraphResolver = async (
    obj,
    args,
    { ksqlDB },
    info
    ...
    if (!sql) {
      return Promise.resolve(
        new Error("Unable to generate ksqlDB from graphql statement.")
      );
    }
   ...
   */
  handleMutationResolve: KsqlDBGraphResolver = async (
    obj,
    args,
    { ksqlDB },
    info
