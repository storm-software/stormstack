...
const generateGraphQLArgs = (fields: any): any =>
  Object.keys(fields).reduce((ksqlEntities: any, filter: any) => {
    if (isInputType(fields[filter].type)) {
      ksqlEntities[filter] = fields[filter];
    }
    return ksqlEntities;
  }, {});
...
function generateQueries(
  ...
  return (ksqlEntities: { [name: string]: any }, query: any): any => {
    ...
    if (ksqlDBQuery != null && ksqlDBQuery.writeQueries.length > 0) {
      ...
      if (subscriptionFields[query.name] != null) {
        ksqlEntities[query.name] = subscriptionFields[query.name];
      } else {
  ...
  return {
    ...
    fields: {
      queryFields: Object.keys(queryFields)
        ...
        .reduce((ksqlEntities: any, key: string) => {
          ksqlEntities[key] = queryFields[key];
          return ksqlEntities;
        }, {}),
      subscriptionFields,
      mutationFields,
