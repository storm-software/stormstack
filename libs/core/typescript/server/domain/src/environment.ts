...
for (const configItem of Object.values(config)) {
  if (configItem.success === false) {
    environmentErrors.push(JSON.stringify(configItem.error.format(), null, 4));
  }
}
...
function extractConfig<Input, Output>(
  config: zod.SafeParseReturnType<Input, Output>
): Output {
  if (!config.success) {
...
const base = extractConfig(config.base);
const postgres = extractConfig(config.postgres);
const sentry = extractConfig(config.sentry);
const clickhouse = extractConfig(config.clickhouse);
const redis = extractConfig(config.redis);
const supertokens = extractConfig(config.supertokens);
const github = extractConfig(config.github);
const prometheus = extractConfig(config.prometheus);
const log = extractConfig(config.log);
const cdnCf = extractConfig(config.cdnCf);
const cdnApi = extractConfig(config.cdnApi);
const authLegacyAuth0 = extractConfig(config.authLegacyAuth0);
const hive = extractConfig(config.hive);
const s3 = extractConfig(config.s3);
...
const hiveConfig =
  hive.HIVE === "1"
    ? {
        ...
        reporting:
          hive.HIVE_REPORTING === "1"
            ? { endpoint: hive.HIVE_REPORTING_ENDPOINT ?? null }
            : null,
        usage:
          hive.HIVE_USAGE === "1"
