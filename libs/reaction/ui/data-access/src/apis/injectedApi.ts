...
import { reactionApiSlice as api } from "./apiSlice";
  ...
  .injectEndpoints({
    endpoints: (build: EndpointBuilder<HttpHandler, "Reaction", "reactionApi">) => ({
      addReaction: build.mutation<CommandSuccessResponse, ReactionApiAddReactionRequest>({
        ...
        transformResponse: async (responseContext: ResponseContext<CommandSuccessResponse>, meta: any, arg: any): Promise<CommandSuccessResponse> => await ReactionApiResponseProcessor.addReaction(responseContext)
      }),
      getReactions: build.query<GetReactions200Response, ReactionApiGetReactionsRequest>({
        query: queryArg => ReactionApiRequestFactory.getReactions(queryArg),
        providesTags: ["Reaction"],
        transformResponse: async (responseContext: ResponseContext<GetReactions200Response>, meta: any, arg: any): Promise<GetReactions200Response> => await ReactionApiResponseProcessor.getReactions(responseContext)
      }),
      getReactionsCount: build.query<GetReactionsCount200Response, ReactionApiGetReactionsCountRequest>({
        query: queryArg => ReactionApiRequestFactory.getReactionsCount(queryArg),
        providesTags: ["Reaction"],
        transformResponse: async (responseContext: ResponseContext<GetReactionsCount200Response>, meta: any, arg: any): Promise<GetReactionsCount200Response> => await ReactionApiResponseProcessor.getReactionsCount(responseContext)
      }),
      removeReaction: build.mutation<CommandSuccessResponse, ReactionApiRemoveReactionRequest>({
        query: queryArg => ReactionApiRequestFactory.removeReaction(queryArg),
        invalidatesTags: ["Reaction"],
        transformResponse: async (responseContext: ResponseContext<CommandSuccessResponse>, meta: any, arg: any): Promise<CommandSuccessResponse> => await ReactionApiResponseProcessor.removeReaction(responseContext)
      })
    }),
