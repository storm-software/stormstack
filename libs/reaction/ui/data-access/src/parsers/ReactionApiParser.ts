...
export class ReactionApiRequestFactory {
  ...
  public static addReaction({
    ...
    if (
      (isString(contentId) && !isStringSet(contentId)) ||
      isEmpty(contentId)
    ) {
      throw new RequiredError("ReactionApi", "addReaction", "contentId");
    }
    if (isEmptyObject(body)) {
      throw new RequiredError("ReactionApi", "addReaction", "body");
    }

    // Make Request Context
    const requestContext = RequestContext.create(
      "/reactions/{contentId}".replace(
        "{" + "contentId" + "}",
  ...
  public static getReactions({
    ...
    if (
      ...
      throw new RequiredError("ReactionApi", "getReactions", "contentId");
    }
    // verify required parameter 'pageNumber' is not null or undefined
    if (
      (isString(pageNumber) && !isStringSet(pageNumber)) ||
      isEmpty(pageNumber)
    ) {
      throw new RequiredError("ReactionApi", "getReactions", "pageNumber");
    }
    // verify required parameter 'pageSize' is not null or undefined
    if ((isString(pageSize) && !isStringSet(pageSize)) || isEmpty(pageSize)) {
      throw new RequiredError("ReactionApi", "getReactions", "pageSize");
    }
    // verify required parameter 'orderBy' is not null or undefined
    if ((isString(orderBy) && !isStringSet(orderBy)) || isEmpty(orderBy)) {
      throw new RequiredError("ReactionApi", "getReactions", "orderBy");
    }
    ...
    if (pageNumber !== undefined) {
      requestContext.setQueryParam(
        "pageNumber",
        ObjectSerializer.serialize(pageNumber, "number", "", enumsMap, typeMap)
      );
    }
    ...
    if (pageSize !== undefined) {
      requestContext.setQueryParam(
        "pageSize",
        ObjectSerializer.serialize(pageSize, "number", "", enumsMap, typeMap)
      );
    }
    ...
    if (orderBy !== undefined) {
      requestContext.setQueryParam(
        "orderBy",
        ObjectSerializer.serialize(orderBy, "string", "", enumsMap, typeMap)
      );
    }
   ...
   * @param type The type of reaction the user had
   */
  public static getReactionsCount({
    contentId = undefined,
    type = undefined,
  }: {
    contentId?: string;
    ...
    if (
      ...
      throw new RequiredError("ReactionApi", "getReactionsCount", "contentId");
    }

    // Make Request Context
    const requestContext = RequestContext.create(
      "/reactions/{contentId}/count".replace(
        "{" + "contentId" + "}",
        encodeURIComponent(String(contentId))
      ),
      HttpMethod.GET
  ...
  public static removeReaction({
    ...
    if (
      ...
      throw new RequiredError("ReactionApi", "removeReaction", "contentId");
    }
...
export class ReactionApiResponseProcessor {
  ...
  public static async addReaction(
    response: ResponseContext<CommandSuccessResponse>
    ...
    if (isCodeInRange("401", response.httpStatusCode)) {
      const body: ProblemDetailsResponse = ObjectSerializer.deserialize(
        ...
        enumsMap,
        typeMap
      ) as ProblemDetailsResponse;
      throw new ApiException<ProblemDetailsResponse>(
  ...
  public static async getReactions(
    response: ResponseContext<GetReactions200Response>
   ...
   * @throws ApiException if the response code was not in [200, 299]
   */
  public static async getReactionsCount(
    response: ResponseContext<GetReactionsCount200Response>
    ...
    if (isCodeInRange("200", response.httpStatusCode)) {
      const body: GetReactionsCount200Response = ObjectSerializer.deserialize(
        ...
        enumsMap,
        typeMap
      ) as GetReactionsCount200Response;
      return body;
    }
    if (isCodeInRange("401", response.httpStatusCode)) {
      const body: ProblemDetailsResponse = ObjectSerializer.deserialize(
