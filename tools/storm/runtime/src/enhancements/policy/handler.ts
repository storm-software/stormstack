 ...
 */
export class PolicyProxyHandler<DbClient extends DbClientContract>
  implements PrismaProxyHandler
{
  ...
  async findUniqueOrThrow(args: any) {
    ...
    if (this.shouldLogQuery) {
      this.logger.info(
        `[policy] \`findUniqueOrThrow\` ${this.model}:\n${formatObject(args)}`
      );
    }
  ...
  async findFirstOrThrow(args: any) {
    ...
    if (this.shouldLogQuery) {
      this.logger.info(
        `[policy] \`findFirstOrThrow\` ${this.model}:\n${formatObject(args)}`
      );
    }
  ...
  async create(args: any) {
    ...
    const inputCheck = this.utils.checkInputGuard(
      this.model,
      args.data,
      "create"
    );
    ...
    const { result, error } = await this.transaction(async tx => {
      if (
        ...
        if (this.shouldLogQuery) {
          this.logger.info(
            `[policy] \`create\` ${this.model}: ${formatObject(createArgs)}`
          );
        }
        ...
        const { result, postWriteChecks } = await this.doCreate(
          this.model,
          args,
          tx
        );
  ...
  private async doCreate(
    ...
    const visitor = new NestedWriteVisitor(this.modelMeta, {
      ...
      createMany: async (model, args, context) => {
        enumerate(args.data).forEach(item =>
          this.validateCreateInputSchema(model, item)
        );
        pushIdFields(model, context);
      },
      ...
      connectOrCreate: async (model, args, context) => {
        if (!args.where) {
          throw this.utils.validationError(
            `'where' field is required for connectOrCreate`
          );
        }
        ...
        if (existing) {
          ...
          if (context.field?.backLink) {
            const backLinkField = resolveField(
              this.modelMeta,
              model,
            ...
            if (backLinkField?.isRelationOwner) {
              ...
              await this.utils.checkPolicyForUnique(
                model,
                args.where,
                "update",
                db
      ...
      connect: async (model, args, context) => {
        if (
          ...
          throw this.utils.validationError(
            `'connect' field must be an non-empty object`
          );
        }
    ...
    if (this.shouldLogQuery) {
      this.logger.info(
        `[policy] \`create\` ${model}: ${formatObject(createArgs)}`
      );
    }
  ...
  private validateCreateInputSchema(model: string, data: any) {
    ...
    if (schema) {
      const parseResult: any = schema.safeParse(data);
  ...
  async createMany(args: { data: any; skipDuplicates?: boolean }) {
    ...
    if (!needPostCreateCheck) {
      ...
      return this.transaction(async tx => {
        const { result, postWriteChecks } = await this.doCreateMany(
          this.model,
          args,
          tx
        );
  ...
  private async doCreateMany(
    ...
    let createResult = await Promise.all(
      enumerate(args.data).map(async item => {
        if (args.skipDuplicates) {
          ...
          for (const constraint of Object.values(uniqueConstraints)) {
            if (constraint.fields.every(f => item[f] !== undefined)) {
              ...
              const existing = await this.utils.checkExistence(
                db,
                model,
                uniqueFilter
              );
              if (existing) {
                if (this.shouldLogQuery) {
                  this.logger.info(
                    `[policy] skipping duplicate ${formatObject(item)}`
                  );
                }
                return undefined;
              }
        ...
        if (this.shouldLogQuery) {
          this.logger.info(
            `[policy] \`create\` ${model}: ${formatObject(item)}`
          );
        }
  ...
  private async doUpdate(args: any, db: Record<string, DbOperations>) {
    ...
    const _registerPostUpdateCheck = async (
      ...
      if (
        this.utils.hasAuthGuard(model, "postUpdate") ||
        this.utils.getZodSchema(model)
      ) {
    ...
    const _create = async (
      ...
      const { postWriteChecks: checks } = await this.doCreate(
        model,
        { data: createData },
        db
      );
    ...
    const _createMany = async (
      ...
      // proceed with the create and collect post-create checks
      const { postWriteChecks: checks } = await this.doCreateMany(
        model,
        args,
        db
      );
    ...
    const _connectDisconnect = async (
      ...
      if (context.field?.backLink) {
        const backLinkField = this.utils.getModelField(
          model,
          context.field.backLink
        );
        if (backLinkField.isRelationOwner) {
    ...
    const visitor = new NestedWriteVisitor(this.modelMeta, {
      update: async (model, args, context) => {
        ...
        if (thisModelUpdate) {
          ...
          for (const key of Object.keys(existing)) {
            const updateValue = (args as any).data
              ? (args as any).data[key]
              : (args as any)[key];
            if (
      ...
      connect: async (model, args, context) =>
        _connectDisconnect(model, args, context),
      ...
      set: async (model, args, context) => {
        ...
        await Promise.all(
          currentSet.map(item => _connectDisconnect(model, item, context))
        );

        // proceed with connecting the new set
        await Promise.all(
          enumerate(args).map(item => _connectDisconnect(model, item, context))
        );
      },
    ...
    if (this.shouldLogQuery) {
      this.logger.info(
        `[policy] \`update\` ${this.model}: ${formatObject(args)}`
      );
    }
  ...
  async updateMany(args: any) {
    ...
    if (
      this.utils.hasAuthGuard(this.model, "postUpdate") ||
      this.utils.getZodSchema(this.model)
    ) {
      ...
      if (this.shouldLogQuery) {
        this.logger.info(
          `[policy] \`updateMany\` ${this.model}: ${formatObject(args)}`
        );
      }
  ...
  async delete(args: any) {
    ...
    const { result, error } = await this.transaction(async tx => {
      ...
      const r = await this.utils.readBack(
        tx,
        this.model,
        "delete",
        args,
  ...
  private transaction(
    action: (tx: Record<string, DbOperations>) => Promise<any>
  ) {
  ...
  private async runPostWriteChecks(
    postWriteChecks: PostWriteCheckRecord[],
    db: Record<string, DbOperations>
  ) {
    await Promise.all(
      postWriteChecks.map(
        async ({ model, operation, uniqueFilter, preValue }) =>
          this.utils.checkPolicyForUnique(
            model,
            uniqueFilter,
            operation,
            db,
