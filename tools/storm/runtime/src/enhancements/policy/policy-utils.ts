...
import { lowerCaseFirst, upperCaseFirst } from "@open-system/core-shared-utilities/common/string-fns";
...
export class PolicyUtil {
  ...
  checkInputGuard(
    ...
    const provider: InputCheckFunc | boolean | undefined =
      guard[`${operation}_input` as const];
  ...
  private async injectGuardForToManyField(
    ...
    if (payload.some) {
      await this.injectGuardForRelationFields(
        fieldInfo.type,
        payload.some,
        operation
      );
  ...
  async injectForRead(model: string, args: any) {
    ...
    if (
      injected.where &&
      Object.keys(injected.where).length > 0 &&
      !this.isTrue(injected.where)
    ) {
    ...
    const hoistedConditions = await this.injectNestedReadConditions(
      model,
      args
    );
  ...
  private flattenGeneratedUniqueField(model: string, args: any) {
    ...
    const uniqueConstraints =
      this.modelMeta.uniqueConstraints?.[lowerCaseFirst(model)];
    ...
    for (let i = context.nestingPath.length - 1; i >= 0; i--) {
      ...
      if (!result) {
        ...
        if (!currField.backLink) {
          throw this.unknownError(
            `field ${currField.type}.${currField.name} doesn't have a backLink`
          );
        }
        const backLinkField = this.getModelField(
          currField.type,
          currField.backLink
        );
        if (backLinkField?.isArray) {
          ...
          if (
            where &&
            backLinkField.isRelationOwner &&
            backLinkField.foreignKeyMapping
          ) {
            for (const [r, fk] of Object.entries<string>(
              backLinkField.foreignKeyMapping
            )) {
  ...
  private async injectNestedReadConditions(
    ...
    for (const field of getModelFields(injectTarget)) {
      ...
      // recurse
      const subHoisted = await this.injectNestedReadConditions(
        fieldInfo.type,
        injectTarget[field]
      );
  ...
  async checkPolicyForUnique(
    ...
    const schema = ["create", "postUpdate"].includes(operation)
      ? this.getZodSchema(model)
      : undefined;
    ...
    if (this.shouldLogQuery) {
      this.logger.info(
        `[policy] checking ${model} for ${operation}, \`findFirst\`:\n${formatObject(
          query
        )}`
      );
    }
    ...
    if (schema) {
      ...
      const parseResult: any = schema.safeParse(result);
      if (!parseResult?.success) {
        const error = fromZodError(parseResult?.error);
        if (this.logger.enabled("info")) {
          this.logger.info(
            `entity ${model} failed validation for operation ${operation}: ${error}`
          );
        }
        throw this.deniedByPolicy(
          model,
  ...
  async checkExistence(
    ...
    if (this.shouldLogQuery) {
      this.logger.info(
        `[policy] checking ${model} existence, \`findFirst\`:\n${formatObject(
          uniqueFilter
        )}`
      );
    }
  ...
  async readBack(
    ...
    if (this.shouldLogQuery) {
      this.logger.info(
        `[policy] checking read-back, \`findFirst\` ${model}:\n${formatObject(
          readArgs
        )}`
      );
    }
  ...
  deniedByPolicy(
    ...
    return prismaClientKnownRequestError(
      this.db,
      `denied by policy: ${model} entities failed '${operation}' check${
        extra ? ", " + extra : ""
      }`,
      {
  ...
  private hasFieldValidation(model: string): boolean {
    return (
      this.policy.validation?.[lowerCaseFirst(model)]?.hasValidation === true
    );
  }
  ...
  getZodSchema(
    ...
    const schemaKey = `${upperCaseFirst(model)}${
      kind ? upperCaseFirst(kind) : ""
    }Schema`;
