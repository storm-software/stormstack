...
export function withPolicy<DbClient extends object>(
  ...
  if (prismaVer && semver.lt(prismaVer, PRISMA_MINIMUM_VERSION)) {
    console.warn(
      `Storm requires Prisma version "${PRISMA_MINIMUM_VERSION}" or higher. Detected version is "${prismaVer}".`
    );
  }
...
function getDefaultPolicy(): PolicyDef {
  try {
    return require(".storm/policy").default;
  } catch {
    if (process.env.STORM_TEST === "1") {
      try {
        ...
        throw new Error(
          'Policy definition cannot be loaded from default location. Please make sure "storm generate" has been run.'
        );
      }
    }
    throw new Error(
