...
export function withPassword<DbClient extends object = any>(
  ...
  return makeProxy(
    ...
    _modelMeta,
    (_prisma, model) =>
      new PasswordHandler(_prisma as DbClientContract, model, _modelMeta),
    "password"
  );
}
...
class PasswordHandler extends DefaultPrismaProxyHandler {
  ...
  protected async preprocessArgs(action: PrismaProxyActions, args: any) {
    ...
    if (args && args.data && actionsOfInterest.includes(action)) {
      await this.preprocessWritePayload(
        this.model,
        action as PrismaWriteActionType,
        args
      );
    }
    return args;
  ...
  private async preprocessWritePayload(
    ...
    const visitor = new NestedWriteVisitor(this.modelMeta, {
      field: async (field, _action, data, context) => {
        const pwdAttr = field.attributes?.find(
