...
/**
 * Validates input declarations.
 */
export default class InputValidator implements AstValidator<Input> {
  validate(dm: Input, accept: ValidationAcceptor): void {
  ...
  private validateFields(dm: Input, accept: ValidationAcceptor) {
    /*const idFields = dm.$resolvedFields.filter(f =>
    ...
    const modelUniqueFields = getUniqueFields(dm);
    ...
    if (
      ...
      if (allows.length > 0 || denies.length > 0 || hasFieldValidation) {
        ...
        accept(
          "error",
          "Input must include a field with @id or @unique attribute, or a model-level @@id or @@unique attribute to use access policies",
          {
            node: dm
          }
        );
      ...
      accept(
        "error",
        "Input cannot have both field-level @id and model-level @@id attributes",
        {
          node: dm
        }
      );
      ...
      accept(
        "error",
        "Input can include at most one field with @id attribute",
        {
          node: dm
        }
      );
  ...
  private validateAttributes(dm: Input, accept: ValidationAcceptor) {
  ...
  private isSelfRelation(field: DataModelField, relationName?: string) {
    ...
    if (relationName) {
      ...
      // points back
      const oppositeModel = field.type.reference?.ref as Input;
      if (oppositeModel) {
        const oppositeModelFields =
          oppositeModel.$resolvedFields as DataModelField[];
  ...
  private validateRelationField(
    ...
    if (oppositeFields.length === 0) {
      ...
      if (field.$isInherited) {
        info.property = "name";
        const container = field.$container as Input;
      ...
      const data: MissingOppositeRelationData = {
        ...
        relationFieldDocUri,
        inputName: field.$container.name
      };
    ...
    if (relationOwner !== field && !relationOwner.type.array) {
      ...
      const containingModel = field.$container as Input;
  ...
  private validateBaseAbstractModel(model: Input, accept: ValidationAcceptor) {
    model.superTypes.forEach((superType, index) => {
      if (!superType.ref?.isAbstract)
        accept(
          "error",
          `Input ${superType.$refText} cannot be extended because it's not abstract`,
          {
            node: model,
            property: "superTypes",
            index
