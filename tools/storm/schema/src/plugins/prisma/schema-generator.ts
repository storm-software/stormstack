...
import { getPrismaVersion, PRISMA_MINIMUM_VERSION } from "@open-system/tools-storm-runtime";
import {
  analyzePolicies,
  getDataModels,
  getDMMF,
...
export default class PrismaSchemaGenerator {
  private zModelGenerator: StormCodeGenerator = new StormCodeGenerator();
...
`;
  ...
  private generateModel(
    ...
    decl.attributes
      ...
      .forEach(attr =>
        model.addComment("/// " + this.zModelGenerator.generateAttribute(attr))
      );
  ...
  private generateModelField(model: PrismaDataModel, field: DataModelField) {
    ...
    const documentations = nonPrismaAttributes.map(
      attr => "/// " + this.zModelGenerator.generateAttribute(attr)
    );
  ...
  private generateEnum(prisma: PrismaModel, decl: Enum) {
    ...
    decl.attributes
      .filter(attr => attr.decl.ref && !this.isPrismaAttribute(attr))
      .forEach(attr =>
        _enum.addComment("/// " + this.zModelGenerator.generateAttribute(attr))
      );
