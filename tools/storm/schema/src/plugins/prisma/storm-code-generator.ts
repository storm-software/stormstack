 ...
 */
export interface StormCodeOptions {
  binaryExprNumberOfSpaces: number;
  unaryExprNumberOfSpaces: number;
}

export default class StormCodeGenerator {
  private readonly options: StormCodeOptions;
  ...
  generateObjectExpr(ast: ObjectExpr) {
    return `{ ${ast.fields
      .map(field => this.generateObjectField(field))
      .join(", ")} }`;
  }
  ...
  generateArrayExpr(ast: ArrayExpr) {
    return `[${ast.items
      .map(item => this.generateExpression(item))
      .join(", ")}]`;
  }
  ...
  generateUnaryExpr(ast: UnaryExpr) {
    return `${ast.operator}${this.unaryExprSpace}${this.generateExpression(
      ast.operand
    )}`;
  }
  ...
  generateBinaryExpr(ast: BinaryExpr) {
    ...
    return `${isLeftParenthesis ? "(" : ""}${this.generateExpression(
      ast.left
    )}${isLeftParenthesis ? ")" : ""}${this.binaryExprSpace}${operator}${
