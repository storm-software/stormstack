...
describe("To theme ui", () => {
  ...
  it("Get tokens - apply parsers - json", async () => {
    ...
    result.fontSizes.reduce(
      (lastValue: number, currentValue: string | number) => {
    ...
    Object.values(result.zIndices as ObjectOfStringNumber).forEach(
      (item: number) => expect(item).toEqual(expect.any(Number))
    );
    (
      [
        "durations",
      ...
      Object.values(result[key] as ObjectOfStringString).forEach(
        (item: string) => expect(item).toEqual(expect.any(String))
      );
    });
    (["fontWeights", "zIndices"] as Array<ThemeUiType>).forEach(key => {
      Object.values(result[key] as ObjectOfStringNumber).forEach(
        (item: number) => expect(item).toEqual(expect.any(Number))
      );
    });
  ...
  it("Get tokens - apply parsers - with variant", async () => {
    ...
    Object.values(
      result.text as Record<string, { fontFamily: string }>
    ).forEach(value => {
  ...
  it("Should return variant that matched nothing", async () => {
    const tokens = seeds().tokens.filter(
      ({ type }) => type !== "measurement" && type !== "color"
    );
    const str = await toThemeUi(
      tokens,
      {
  ...
  it("Should return tokens wihtout any transformation on name", async () => {
    ...
    expect(tokens.map(({ name }) => name)).toEqual(
      expect.arrayContaining(themeUiTokensNameList)
    );
  });
