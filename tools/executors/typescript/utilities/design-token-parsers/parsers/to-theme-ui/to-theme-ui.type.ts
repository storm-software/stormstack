...
export type ThemeUiIndexes = Partial<
  Record<ThemeUiType, Record<string, string | number>>
>;
export interface ThemeUiTokenClass {
  new (
    tokens: Partial<Token>,
    transformNameFn: Function
  ): ThemeUiTokenClassInstance;
  afterGenerate?(
    themeUiTokens: Record<ThemeUiTypes, object>
  ): Record<ThemeUiTypes, object>;
  generateVariants?(
...
export interface ThemeUiTokenClassInstance {
  ...
  ): Partial<Record<ThemeUiTypes, any>>;
}
