...
describe("To React Native", () => {
  ...
  it("Should use assetsFolderPath", async () => {
    const tokens = seeds().tokens;
    expect(
      await toReactNative(tokens, { assetsFolderPath: "path" }, libs)
    ).toMatchSnapshot();
    expect(
      await toReactNative(tokens, { assetsFolderPath: "./path" }, libs)
    ).toMatchSnapshot();
  ...
  it("Should support different formatName options", async () => {
    (
      ["camelCase", "kebabCase", "snakeCase", "pascalCase", "none"] as const
  ...
  it("Should support different formatKey options", async () => {
    (["camelCase", "kebabCase", "snakeCase", "pascalCase"] as const).map(
      async formatKeys => {
        const tokens = seeds().tokens;
        const result = await toReactNative(
          tokens,
          { assetsFolderPath: "path", formatKeys },
          libs
        );
        expect(result).toMatchSnapshot();
      }
