...
export class Border extends BorderToken {
  ...
  generate(options: OptionsType): BorderMapping {
    ...
    if (width && width.value) {
      result.borderWidth = Utils.go<
        ConstructorParameters<typeof BorderToken>[0]
      >(
        this.token,
        options,
        "borderWidth",
    ...
    if (color && color.value) {
      ...
      if (color.value.a && color.value.a !== 1) {
        result.borderOpacity = Utils.go<
          ConstructorParameters<typeof BorderToken>[0]
        >(this.token, options, "borderOpacity", `${color.value.a}`);
      }
    }
