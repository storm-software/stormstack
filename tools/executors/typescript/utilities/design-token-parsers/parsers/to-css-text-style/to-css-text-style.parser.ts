...
export type InputDataType = Array<
  { name: string; value: TextStyleValue } & Record<any, any>
>;
...
type Properties =
  | { include: Array<FilterList> }
  | { exclude: Array<FilterList> };
type ColorsFormat =
  | "rgb"
  | "prgb"
  | "hex"
...
class ToCssTextStyle {
  ...
  constructor(
    ...
    this.fontName =
      (textStyle.font as FontToken).name ?? this.font.fontPostScriptName;
  ...
  trigger = {
    font: () => {
      if (this.isIncluded("font-family")) {
        let fontFamily = applyTransformStr(
          this.fontName,
          this.options?.fontFamilyFormat
        );
        if (this.options?.genericFamily)
          fontFamily += `, ${this.options.genericFamily}`;
    ...
    fontSize: () => {
      if (this.isIncluded("font-size")) {
        this.cssContent.push(
          `font-size: ${this.fontSize?.measure}${this.fontSize?.unit}`
        );
      }
    },
    ...
    color: () => {
      if (this.isIncluded("color")) {
        this.cssContent.push(
          `color: ${tinycolor(this.color).toString(
            this.options?.colorFormat || "rgb"
          )}`
        );
      }
    },
    lineHeight: () => {
      if (this.isIncluded("line-height") && this.lineHeight) {
        ...
        if (this.options?.relativeLineHeight) {
          ...
          lineHeight = `${this.lineHeight?.measure}${
    ...
    letterSpacing: () => {
      if (this.isIncluded("letter-spacing")) {
        this.cssContent.push(
          `letter-spacing: ${this.letterSpacing?.measure}${
            this.letterSpacing?.unit || ""
          }`
        );
      }
    },
    ...
    textDecoration: () => {
      if (this.isIncluded("text-decoration") && this.textDecoration) {
        this.cssContent.push(
          `text-decoration: ${this.textDecoration.join(" ")}`
        );
      }
    },
    textIndent: () => {
      if (this.isIncluded("text-indent")) {
        this.cssContent.push(
          `text-indent: ${this.textIndent?.measure}${this.textIndent?.unit}`
        );
      }
    },
    fontVariant: () => {
...
const getTextStyleProperties = function (options?: OptionsType) {
  ...
  if (
    ...
    textStyleProperties = textStyleProperties.filter(property => {
      return (
        propertiesBase.includes(property) &&
...
const getCssProperties = function (options?: OptionsType) {
  ...
  if (
    ...
    cssProperties = options.include.filter(property => {
      return propertiesBaseInCss.includes(
        property as FilterListFromCssProperties
      );
...
export default async function (
  ...
  try {
    ...
    const result = inputData
      .map(token => {
        const toCssTextStyle = new ToCssTextStyle(
          token.value,
          token.name,
          cssProperties,
          options
        ...
        textStylesProperties.forEach(property => {
          if (
            property in toCssTextStyle &&
            !!toCssTextStyle[property as keyof TextStyleValue]
          ) {
            toCssTextStyle.trigger[
              property as keyof ToCssTextStyle["trigger"]
            ]();
          }
        });
        ...
        let name = applyTransformStr(
          token.name,
          options?.cssClassFormat || "kebabCase"
        );
