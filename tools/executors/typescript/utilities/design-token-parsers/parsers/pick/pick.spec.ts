...
describe("Pick", () => {
  it("Execute parser", async () => {
    ...
    result.forEach(token =>
      expect(Object.keys(token)).toEqual(["name", "type"])
    );
    return;
  });
  ...
  it("Execute parser with flatten option", async () => {
    ...
    tokens.forEach(token => {
      ...
      expect(Object.keys(result.find(({ id }) => id === token.id)!)).toEqual(
        expectation
      );
    });
    return;
  ...
  it("Execute parser with filter", async () => {
    ...
    result.forEach(token => {
      if (token.type === "textStyle")
        expect(has(token, "value.color")).toEqual(false);
      if (token.type === "border")
        expect(has(token, "value.color")).toEqual(true);
    });
    return;
  });
});
