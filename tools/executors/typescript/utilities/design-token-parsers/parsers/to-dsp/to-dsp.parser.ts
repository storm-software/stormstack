...
export async function createAssetsFiles(
  ...
  const filteredTokens = tokens.filter(token =>
    assetsTypes.includes(token.type)
  );
...
export default async function (
  ...
  const filesToCreate = await Object.entries(filesToCreateForTokenTypes).reduce(
    async (accPromise, [fileName, types]) => {
      ...
      const entities = tokens
        .filter(token =>
          types.length ? (types as Array<string>).includes(token.type) : false
        )
        .map((token): DspEntity => {
          const instance = new (<any>TokensClass)[
            `${token.type.charAt(0).toUpperCase() + token.type.slice(1)}`
  ...
  if (options && options.settings) {
    dspJson.settings = {
      ...
      build_status_label:
        options.settings.buildStatusLabel || defaultSettings.build_status_label,
      package_version:
        options.settings.packageVersion || defaultSettings.package_version,
      snippet_trigger_prefix:
        options.settings.snippetTriggerPrefix ||
        defaultSettings.snippet_trigger_prefix,
    };
  }
