...
const entitiesLength = seeds().tokens.filter(({ type }) =>
  entitiesAvailable.includes(type)
).length;
...
describe("To DSP", () => {
  it("should take settings and be able to return the correct value", async () => {
    ...
    expect(result.length).toBe(
      tokenSeeds.filter(({ type }) => type === "vector" || type === "bitmap")
        .length + 5
    );
    expect(result.some(entity => entity.name === "dsp.json")).toBe(true);
    expect(result.some(entity => entity.name === "data/tokens.json")).toBe(
      true
    );
    expect(result.some(entity => entity.name === "data/components.json")).toBe(
      true
    );
    ...
    const components = result.find(
      entity => entity.name === "data/components.json"
    );
    ...
    expect(dspContent.settings.build_status_label).toEqual(
      settings.buildStatusLabel
    );
    expect(dspContent.settings.package_version).toEqual(
      settings.packageVersion
    );
    expect(dspContent.settings.snippet_trigger_prefix).toEqual(
      settings.snippetTriggerPrefix
    );

    expect(
      dspContent.import.some(
        (imported: { src: string }) => imported.src === "data/tokens.json"
      )
    ).toBe(true);
    expect(
      dspContent.import.some(
    ...
    expect(
      dspContent.import.some(
        (imported: { src: string }) => imported.src === "data/fonts.json"
      )
    ).toBe(true);
    expect(
      dspContent.import.some(
        (imported: { src: string }) => imported.src === "data/docs.json"
      )
    ).toBe(true);
  });
