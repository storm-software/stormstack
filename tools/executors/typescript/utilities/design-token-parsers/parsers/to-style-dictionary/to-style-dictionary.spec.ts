...
describe("To Style Dictionary", () => {
  it("Should be able to extract tokens in multiple files", async () => {
    ...
    const resultWithOnlyColors = result.filter(file =>
      file.name.includes("color/")
    );
  ...
  it("Should be able to extract Color token type", async () => {
    ...
    expect(content.color.base.colors.accent).toEqual({
      value: expect.any(String),
    });
    expect(content.color.base.colors.black).toEqual({
      value: expect.any(String),
    });
    expect(content.color.base.colors.green).toEqual({
      value: expect.any(String),
    });
    expect(content.color.base.colors.grey).toEqual({
      value: expect.any(String),
    });
    expect(content.color.base.colors.orange).toEqual({
      value: expect.any(String),
    });
  });
  ...
  it("Should be able to extract Depth token type", async () => {
    ...
    expect(content.depth.base.background).toEqual({
      value: expect.any(String),
    });
    expect(content.depth.base.middle).toEqual({ value: expect.any(String) });
    expect(content.depth.base.foreground).toEqual({
      value: expect.any(String),
    });
  });
  ...
  it("Should be able to extract Measurement token type", async () => {
    ...
    Object.values(content.size).forEach(property => {
      Object.values(property as Record<string, { value: string }>).forEach(
        value => {
  ...
  it("Should be able to extract Opacity token type", async () => {
    ...
    expect(content.opacity.base.transparent).toEqual({
      value: expect.any(String),
    });
      ...
      Object.values(content).forEach(value => {
        ...
        Object.values(
          value as NonNullable<
            BaseStyleDictionaryTokensFormat["size" | "color"]
          >
        ).forEach(nestedValue => {
    ...
    result.forEach(file => {
      ...
      Object.values(content).forEach(property => {
        Object.values(property).forEach(nestedProperty => {
          Object.values(
            nestedProperty as Record<string, { value: string }>
          ).forEach(value => {
            expect(value.value).toEqual(expect.any(String));
          });
        });
  ...
  it("Should be able to extract Font token type", async () => {
    ...
    Object.values(content).forEach(property => {
      Object.values(property).forEach(nestedProperty => {
        Object.values(
          nestedProperty as Record<string, Record<string, { value: string }>>
        ).forEach(format => {
  ...
  it("Should be able to extract without splitBy", async () => {
    ...
    expect(Object.keys(content)[0]).toEqual("color");
    expect(Object.keys(content.color)[0]).toEqual("base");
    expect(content.color.base["colorsAccent"]).toEqual({
      value: expect.any(String),
    });
    expect(content.color.base["colorsBlack"]).toEqual({
      value: expect.any(String),
    });
    expect(content.color.base["colorsGreen"]).toEqual({
      value: expect.any(String),
    });
    expect(content.color.base["colorsGrey"]).toEqual({
      value: expect.any(String),
    });
    expect(content.color.base["colorsOrange"]).toEqual({
      value: expect.any(String),
    });
  });
  ...
  describe("Should generate simple token per type", () => {
    it("Color", () => {
      seeds()
        ...
        .map(color => {
          const tokenClass: StyleDictionaryTokenClass = (<any>TokensClass)[
            "Color"
          ];
          const instance = new tokenClass(color, color.name.split("/"));
          const result = instance.generate({
            formatTokens: { colorFormat: { format: "rgb" } },
          });
    ...
    it("TextStyle", () => {
      seeds()
        ...
        .map(textStyle => {
          ...
          const result = instance.generate({});
          const firstLevelSplitedTokenName = textStyle.name
            .split("-")
            .shift() as string;
          ...
          if ("letterSpacing" in textStyle.value) {
            expectation.size.letterSpacing = {
              [firstLevelSplitedTokenName]: expect.any(Object),
            };
          }
          if ("textIndent" in textStyle.value) {
            expectation.size.textIndent = {
              [firstLevelSplitedTokenName]: expect.any(Object),
            };
          }
          expect(result).toEqual(expectation);
        ...
        .map(font => {
          ...
          expect(result).toEqual({
            asset: {
              font: {
                [familyName]: {
                  [subFamilyName]: expectedFormats.reduce<
