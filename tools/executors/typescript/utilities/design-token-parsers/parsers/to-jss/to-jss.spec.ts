...
describe("To jss", () => {
  it("Get tokens - apply parsers", async () => {
    ...
    const shadow = seeds().tokens.find(
    ...
    expect(
      result.includes(`${libs._.camelCase(shadow.name)}: "${shadowValue}",`)
    ).toBe(true);
    expect(
      result.includes(
  ...
  it("Get tokens - apply parsers - with custom jssObjectName", async () => {
    ...
    expect(result.includes(`${options.formatConfig!.jssObjectName}`)).toBe(
      true
    );
    return;
  });
  ...
  it("Get tokens - apply parsers - with values format options", async () => {
    ...
    const border = seeds().tokens.find(
  ...
  it("Module Format - commonjs", async () => {
    ...
    expect(result.includes(`export const ${jssObjectName}`)).toBeFalsy();
    expect(
      result.includes(`module.exports = { ${jssObjectName} }`)
    ).toBeTruthy();
  });
  ...
  it("No assets folder path - type by tokens", async () => {
    ...
    tokens.forEach(token => {
      if (token.type === "depth") {
        expect(
          result.includes(
            `${token.name}: ${(token.value as DepthToken["value"]).depth}`
          )
        ).toBeTruthy();
        ...
        expect(
          result.includes(
            `${token.name}: ${
              (token.value as MeasurementToken["value"]).measure
            }`
          )
        ).toBeTruthy();
        ...
        expect(
          result.includes(
            `${token.name}: ${
              (token.value as OpacityToken["value"]).opacity / 100
            }`
          )
        ).toBeTruthy();
        ...
        expect(
          result.includes(
            `${token.name}: ${(token.value as DurationToken["value"]).duration}`
          )
        ).toBeTruthy();
