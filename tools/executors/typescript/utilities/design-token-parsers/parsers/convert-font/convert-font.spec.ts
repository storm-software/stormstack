...
describe("convert-font", () => {
  it("Default options", async () => {
    const fonts = seeds().tokens.filter(
      ({ type }) => type === "font"
    ) as Array<FontToken>;
    ...
    expect(result.length).toEqual(
      fonts.filter(({ value }) => !value.fontFileMissing).length * 2
    );
    result.forEach(item => {
      expect(typeof item.value.url).toMatch("string");
      expect(
        ["woff2", "woff"].includes(Path.extname(item.value.format).substring(1))
      );
    });
    const fontsWithoutMissing = fonts.filter(
      ({ value }) => !value.fontFileMissing
    );
  ...
  it("Several options", async () => {
    ...
    expect(Array.isArray(result)).toEqual(true);
    expect(result.length).toEqual(
      fonts.filter(({ value }) => !value.fontFileMissing).length
    );
    result.forEach((item, index) => {
      expect(typeof item.value.url).toMatch("string");
      expect(
        options.formats!.includes(
          Path.extname(item.value.format).substring(1) as AllowedFormat
        )
      );
