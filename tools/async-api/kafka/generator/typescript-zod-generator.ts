...
import { ZodRenderer } from "../renderers/zod-renderer";
...
type TypeScriptTypeMapping = TypeMapping<
  TypeScriptEventsOptions,
  TypeScriptDependencyManager
>;

interface TypeScriptEventsRenderCompleteModelOptions {
  exportType: TypeScriptExportType;
}

const TypeScriptDefaultTypeMapping: TypeScriptTypeMapping = {
...
const TypeScriptDefaultConstraints = {
 ...
 */
export class TypeScriptZodGenerator extends AbstractGenerator<
  TypeScriptEventsOptions,
...
> {
  public static defaultOptions: TypeScriptEventsOptions<TypeScriptPreset> = {
    ...
    dependencyManager: () => {
      return new TypeScriptDependencyManager({
        ...TypeScriptZodGenerator.defaultOptions,
  ...
  constructor(options?: DeepPartial<TypeScriptEventsOptions>) {
    const realizedOptions = TypeScriptZodGenerator.getOptions(options);
  ...
  constrainToMetaModel(
    ...
    const optionsToUse = TypeScriptZodGenerator.getOptions({
      ...this.options,
      ...options,
    });
  ...
  async renderCompleteModel(
    ...
    const completeModelOptionsToUse = mergePartialAndDefault(
      TypeScriptZodGenerator.defaultCompleteModelOptions,
...
${modelCode}`;
  ...
  async renderZod(
    ...
    dependencyManagerToUse.addTypeScriptDependency("z", "zod");
