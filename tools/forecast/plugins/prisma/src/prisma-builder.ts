  ...
  toString(): string {
    const restFields =
      this.restFields.length > 0
        ? this.restFields
            .map(f => indentString(`${f.name} = ${JSON.stringify(f.value)}`))
            .join("\n")
        : "";
    return (
...
export class Model extends ContainerDeclaration {
  ...
  toString(): string {
    const auxiliaryFields = this.fields.filter(f =>
      AUXILIARY_FIELDS.includes(f.name)
    );
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const result: any[] = this.fields.filter(
      f => !AUXILIARY_FIELDS.includes(f.name)
    );
...
export class Enum extends ContainerDeclaration {
  ...
  toString(): string {
    return (
      ...
      `enum ${this.name} {\n` +
      indentString(
        [...this.fields, ...this.attributes].map(d => d.toString()).join("\n")
      ) +
      "\n}"
    );
  }
