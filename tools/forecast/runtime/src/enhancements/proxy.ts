  ...
  const proxy = new Proxy(prisma, {
    get: (target: any, prop: string | symbol, receiver: any) => {
      ...
      if (
        typeof prop !== "string" ||
        prop.startsWith("$") ||
        !models.includes(prop.toLowerCase())
      ) {
...
// Filter out @stormstack/tools-forecast-runtime stack (generated by proxy) from stack trace
function cleanCallStack(stack: string, method: string, message: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      line.includes("@stormstack/tools-forecast-runtime") ||
