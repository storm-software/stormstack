 ...
 */
export function withOmit<DbClient extends object>(
  prisma: DbClient,
  ...
  return makeProxy(
    ...
    _modelMeta,
    (_prisma, model) =>
      new OmitHandler(_prisma as DbClientContract, model, _modelMeta),
    "omit"
  );
}
...
class OmitHandler extends DefaultPrismaProxyHandler {
  ...
  // base override
  protected override async processResultEntity<T>(data: T): Promise<T> {
    if (data) {
