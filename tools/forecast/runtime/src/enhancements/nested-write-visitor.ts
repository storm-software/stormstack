      ...
      case "create":
        ...
        for (const item of enumerate(data)) {
          ...
          if (callbackResult !== false) {
            const subPayload =
              typeof callbackResult === "object" ? callbackResult : item;
            await this.visitSubPayload(
              model,
              action,
              subPayload,
              context.nestingPath
      ...
      case "createMany":
        if (data) {
          ...
          if (this.callback.createMany) {
            callbackResult = await this.callback.createMany(
              model,
              data,
              context
            );
          ...
          if (callbackResult !== false) {
            const subPayload =
              typeof callbackResult === "object" ? callbackResult : data.data;
            await this.visitSubPayload(
              model,
              action,
              subPayload,
      ...
      case "connectOrCreate":
        ...
        for (const item of enumerate(data)) {
          ...
          if (this.callback.connectOrCreate) {
            callbackResult = await this.callback.connectOrCreate(
              model,
              item,
              context
            );
          ...
          if (callbackResult !== false) {
            const subPayload =
              typeof callbackResult === "object" ? callbackResult : item.create;
            await this.visitSubPayload(
              model,
              action,
              subPayload,
        ...
        if (this.callback.connect) {
          for (const item of enumerate(data)) {
            const newContext = {
              ...
              nestingPath: [
                ...context.nestingPath,
                { field, model, where: item, unique: true },
              ],
            };
      ...
      case "updateMany":
        ...
        for (const item of enumerate(data)) {
          ...
          if (this.callback.updateMany) {
            callbackResult = await this.callback.updateMany(
              model,
              item,
              context
            );
            ...
            const newContext = {
              ...
              nestingPath: [
                ...context.nestingPath,
                {
                  field,
                  model,
                  where: toplevel ? item.where : item,
                  unique: false,
                },
              ],
            };
  ...
  private async visitSubPayload(
    ...
    for (const field of getModelFields(payload)) {
      ...
      if (fieldInfo.isDataModel) {
        ...
        for (const [subAction, subData] of Object.entries<any>(
          payload[field]
        )) {
