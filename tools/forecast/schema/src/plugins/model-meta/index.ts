...
import type { RuntimeAttribute } from "@stormstack/tools-forecast-runtime";
        ...
        writer.block(() => {
          for (const f of model.fields) {
            ...
            writer.write(`${f.name}: {
                    ...
                    backLink: ${
                      backlink ? "'" + backlink.name + "'" : "undefined"
                    },
                    isRelationOwner: ${isRelationOwner(f, backlink)},
                    foreignKeyMapping: ${
                      fkMapping ? JSON.stringify(fkMapping) : "undefined"
                    }
                },`);
          }
        });
...
function getRelationName(field: DataModelField) {
  ...
  const relName =
    relAttr && relAttr.args?.[0] && getLiteral<string>(relAttr.args?.[0].value);
  return relName;
}
...
function isRelationOwner(
  field: DataModelField,
  backLink: DataModelField | undefined
) {
  ...
  if (
    ...
    !hasAttribute(backLink, "@relation")
  ) {
...
function generateForeignKeyMapping(field: DataModelField) {
  ...
  if (
    !isArrayExpr(fields) ||
    !isArrayExpr(references) ||
    fields.items.length !== references.items.length
  ) {
    return undefined;
  }

  const fieldNames = fields.items.map(item =>
    isReferenceExpr(item) ? item.target.$refText : undefined
  );
  const referenceNames = references.items.map(item =>
    isReferenceExpr(item) ? item.target.$refText : undefined
  );
