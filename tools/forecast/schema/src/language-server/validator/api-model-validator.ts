 ...
 */
export default class ApiModelValidator implements AstValidator<ApiModel> {
  validate(dm: ApiModel, accept: ValidationAcceptor): void {
    validateDuplicatedDeclarations(dm.$resolvedFields, accept);
    this.validateAttributes(dm, accept);
    this.validateFields(dm, accept);
  }

  private validateFields(dm: ApiModel, accept: ValidationAcceptor) {
    ...
    if (
      ...
      if (allows.length > 0 || denies.length > 0 || hasFieldValidation) {
        ...
        accept(
          "error",
          "ApiModel must include a field with @id or @unique attribute, or a model-level @@id or @@unique attribute to use access policies",
          {
            node: dm
          }
        );
      ...
      accept(
        "error",
        "ApiModel cannot have both field-level @id and model-level @@id attributes",
        {
          node: dm
        }
      );
      ...
      accept(
        "error",
        "ApiModel can include at most one field with @id attribute",
        {
          node: dm
        }
      );
  ...
  private validateAttributes(dm: ApiModel, accept: ValidationAcceptor) {
  ...
  private isSelfRelation(field: DataModelField, relationName?: string) {
    ...
    if (relationName) {
      ...
      // points back
      const oppositeModel = field.type.reference?.ref as ApiModel;
      if (oppositeModel) {
  ...
  private validateRelationField(
    ...
    if (oppositeFields.length === 0) {
      ...
      if (field.$isInherited) {
        info.property = "name";
        const container = field.$container as ApiModel;
    ...
    if (relationOwner !== field && !relationOwner.type.array) {
      ...
      const containingModel = field.$container as ApiModel;
