 ...
 */
export default class InterfaceValidator implements AstValidator<Interface> {
  validate(dm: Interface, accept: ValidationAcceptor): void {
    validateDuplicatedDeclarations(dm.$resolvedFields, accept);
    this.validateAttributes(dm, accept);
    this.validateFields(dm, accept);
  }

  private validateFields(dm: Interface, accept: ValidationAcceptor) {
  ...
  private validateAttributes(dm: Interface, accept: ValidationAcceptor) {
  ...
  private isSelfRelation(field: DataModelField, relationName?: string) {
    ...
    if (relationName) {
      ...
      // points back
      const oppositeModel = field.type.reference?.ref as Interface;
      if (oppositeModel) {
  ...
  private validateRelationField(
    ...
    if (oppositeFields.length === 0) {
      ...
      if (field.$isInherited) {
        info.property = "name";
        const container = field.$container as Interface;
      ...
      const data: MissingOppositeRelationData = {
        ...
        relationFieldDocUri,
        interfaceName: field.$container.name
      };
    ...
    if (relationOwner !== field && !relationOwner.type.array) {
      ...
      const containingModel = field.$container as Interface;
...
export interface MissingOppositeRelationData {
  ...
  // the name of Interface that the relation field belongs to.
