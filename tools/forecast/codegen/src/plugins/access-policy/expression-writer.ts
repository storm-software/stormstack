...
/**
 * Utility for writing Storm expression as Prisma query argument objects into a ts-morph writer
 */
export class ExpressionWriter {
  ...
  constructor(
    ...
    private readonly isPostGuard = false
  ) {
  ...
  /**
   * Writes the given Storm expression.
   */
      ...
      this.block(() => {
        if (leftIsFieldAccess && !rightIsFieldAccess) {
          ...
          throw new PluginError(
            name,
            '"in" operator cannot be used with field references on both sides'
          );
        }
      });
    }
  ...
  private writeComparison(expr: BinaryExpr, operator: ComparisonOperator) {
    ...
    if (
      ...
      !fieldAccess.$resolvedType?.nullable
    ) {
      try {
        this.writer.write(
          ...
          this.block(() => {
            ...
            if (dataModel && isAuthInvocation(operand)) {
              ...
              if (!idFields || idFields.length === 0) {
                throw new PluginError(
                  name,
                  `Data model ${dataModel.name} does not have an id field`
                );
              }
  ...
  private isAuthOrAuthMemberAccess(expr: Expression) {
    return (
      ...
      (isMemberAccessExpr(expr) && isAuthInvocation(expr.operand))
    );
  }
  ...
  private isModelTyped(expr: Expression) {
    return isDataModel(expr.$resolvedType?.decl)
      ? (expr.$resolvedType?.decl as DataModel)
      : undefined;
  }
  ...
  private writeLogical(expr: BinaryExpr, operator: "&&" | "||") {
    ...
    if (operator === "&&") {
      // // && short-circuit: left && right -> left ? right : { storm_guard: false }
      ...
      // // || short-circuit: left || right -> left ? { storm_guard: true } : right
      // if (!this.hasFieldAccess(expr.left)) {
      //     this.plain(expr.left);
      //     this.writer.write(' ? ');
