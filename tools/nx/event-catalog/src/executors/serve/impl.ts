...
export default async function* (
  ...
  try {
    ...
    if (!context.workspace || !context.projectName) {
      ConsoleLogger.error(
        `Context is missing workspace or project name. Workspace: ${context.workspace}, Project Name: ${context.projectName} `
      );
      return { success: false };
    } else if (!context.workspace.projects[context.projectName].targets) {
      ConsoleLogger.error(
        `Context is missing targets. Project Config: ${JSON.stringify(
          context.workspace.projects[context.projectName]
        )} `
      );
      return { success: false };
    } else if (!context.workspace.projects[context.projectName].sourceRoot) {
      ConsoleLogger.error(
        `Context is missing sourceRoot. Project Config: ${JSON.stringify(
          context.workspace.projects[context.projectName]
        )} `
      );
      return { success: false };
    ...
    if (!!context.workspace.projects[context.projectName].sourceRoot) {
