...
${leftNodes
  .map(
    node =>
      `l-${node.id}[${node.name}]:::producer-->${centerNode.id}[${centerNode.name}]:::event\n`
  )
  .join("")}
...
${rightNodes
  .map(
    node =>
      `${centerNode.id}[${centerNode.name}]:::event-->r-${node.id}[${node.name}]:::consumer\n`
  )
  .join("")}
${leftNodes
  .map(
    node =>
      `click l-${node.id} href "${node.link}" "Go to ${node.name}" _self\n`
  )
  .join("")}
${rightNodes
  .map(
    node =>
      `click r-${node.id} href "${node.link}" "Go to ${node.name}" _self\n`
  )
  .join("")}
click ${centerNode.id} href "${centerNode.link}" "Go to ${
  centerNode.name
}" _self\n
`;
